...
SLING_ROLE_REGEXP=$(head /tmp/sling-role-regexp.txt)

function generate_config() {
	# get unique list of roles, IPs, ports into a bash array
	UNIQ=$(grep -v '^ *$' | sort | awk -F# '{ print $1 ":" $2 ":" $3}' | sort -u | egrep ${SLING_ROLE_REGEXP})

	echo "# backends generated by $ME using SLING_ROLE_REGEXP=$SLING_ROLE_REGEXP"
	for i in ${UNIQ[@]}
	do
	  # $1:role, $2:IP, $3:port
	  set $(echo $i | tr ':' ' ')
